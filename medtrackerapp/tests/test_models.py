...
class MedicationModelTests(TestCase):
    ...
    def test_adherence_rate_no_logs(self):
        med = Medication.objects.create(
            name="NoLogs", dosage_mg=10, prescribed_per_day=1
        )
        self.assertEqual(med.adherence_rate(), 0.0)

    def test_adherence_rate_all_taken(self):
        med = Medication.objects.create(
            name="AllTaken", dosage_mg=10, prescribed_per_day=2
        )
        now = timezone.now()
    ...
    def test_adherence_rate_partial(self):
        med = Medication.objects.create(
            name="Partial", dosage_mg=10, prescribed_per_day=2
        )
        now = timezone.now()
        DoseLog.objects.create(medication=med, taken_at=now, was_taken=True)
        DoseLog.objects.create(
    ...
    def test_adherence_rate_over_period_valid(self):
        med = Medication.objects.create(
            name="PeriodMed", dosage_mg=50, prescribed_per_day=1
        )
        today = date.today()
    ...
    def test_adherence_rate_over_period_no_logs(self):
        med = Medication.objects.create(
            name="NoLogsPeriod", dosage_mg=10, prescribed_per_day=2
        )
        today = date.today()
    ...
    def test_adherence_rate_over_period_zero_prescribed(self):
        med = Medication.objects.create(
            name="ZeroPrescribed", dosage_mg=10, prescribed_per_day=0
        )
        today = date.today()
    ...
    def test_adherence_rate_over_period_invalid_dates(self):
        med = Medication.objects.create(
            name="InvalidDates", dosage_mg=10, prescribed_per_day=1
        )
        with self.assertRaises(ValueError):
...
class DoseLogModelTests(TestCase):
    ...
    def test_str_not_taken(self):
        ...
        log = DoseLog.objects.create(
            medication=med, taken_at=timezone.now(), was_taken=False
        )
    ...
    def test_doselog_without_taken_at(self):
        med = Medication.objects.create(
            name="MedTest", dosage_mg=10, prescribed_per_day=1
        )
    ...
    def test_doselog_future_date_allowed(self):
        med = Medication.objects.create(
            name="FutureMed", dosage_mg=10, prescribed_per_day=1
        )
        future_time = timezone.now() + timedelta(days=1)
        log = DoseLog.objects.create(
            medication=med, taken_at=future_time, was_taken=True
        )
        self.assertEqual(log.taken_at, future_time)

    def test_doselog_default_was_taken(self):
        med = Medication.objects.create(
            name="DefaultTaken", dosage_mg=10, prescribed_per_day=1
        )
